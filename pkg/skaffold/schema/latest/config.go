...
type GoogleCloudBuild struct {
	...
	AvailableSecrets struct {
		// SecretManager is a list of secret manager secrets you want to use.
		SecretManager []struct {
			// VersionName of the secret manager secret you want to use.
			VersionName string `yaml:"versionName"`
			// Env is the environment variable to map the secret to.
			Env string `yaml:"env"`
		} `yaml:"secretManager,omitempty"`
	} `yaml:"availableSecrets,omitempty"`

	// PreBuildSteps Set the prebuild steps for the cloudbuild docker job.
	PreBuildSteps []struct {
		// Name represents the name of the step.
		Name string `yaml:"name,omitempty"`
		// ID is the id of the step.
		ID string `yaml:"id,omitempty"`
		// Args represents the arguments to the step.
		Args []string `yaml:"args,omitempty"`
		// Env represents the environment variables to set.
		Env []string `yaml:"env,omitempty"`
		// AllowFailure represents whether the step is allowed to fail.
		AllowFailure bool `yaml:"allowFailure,omitempty"`
		// AllowExitCodes represents the exit codes that are allowed to be returned by the step.
		AllowExitCodes []int64 `yaml:"allowExitCodes,omitempty"`
		// Dir represents the working directory for the step.
		Dir string `yaml:"dir,omitempty"`
		// WaitFor represents the steps that this step should wait for.
		WaitFor []string `yaml:"waitFor,omitempty"`
		// Entrypoint represents the entrypoint for the step.
		Entrypoint string `yaml:"entrypoint,omitempty"`
		// SecretEnv represents the environment variables to set from secret manager.
		SecretEnv []string `yaml:"secretEnv,omitempty"`
		// Volumes represents the volumes to mount for the step.
		Volumes []*CBVolume `yaml:"volumes,omitempty"`
		// Timeout represents the timeout for the step.
		Timeout string `yaml:"timeout,omitempty"`
		// Script represents the script to run.
		Script string `yaml:"script,omitempty"`
		// AutomapSubstitutions represents whether to automap substitutions.
		AutomapSubstitutions bool `yaml:"automapSubstitutions,omitempty"`
	} `yaml:"preBuildSteps,omitempty"`

	// BuilderConfig Allow you to plumb secrets to the base builder in gcb builds.
	BuilderConfig struct {
		// SecretEnv represents the environment variables to set from secret manager.
		SecretEnv []string `yaml:"secretEnv,omitempty"`
		// Volumes represents the volumes to mount for the step.
		Volumes []struct {
			// Name represents the name of the volume.
			Name string `yaml:"name"`
			// Path represents the path to the volume.
			Path string `yaml:"path"`
		} `yaml:"volumes,omitempty"`
	} `yaml:"builderConfig,omitempty"`
}

// CBVolume is a volume for the cloudbuild job.
type CBVolume struct {
	// Name of the volume.
	Name string `yaml:"name,omitempty"`
	// Path to the volume.
	Path string `yaml:"path,omitempty"`
...
type TestCase struct {
	...
	// For example: `["./test/*"]`.
	GrypeTests []*GrypeTest `yaml:"grypeTests,omitempty"`

	// TrivyTests lists the Trivy tests to run on that artifact.
	// For example: `["./test/*"]`.
	TrivyTests []*TrivyTest `yaml:"trivyTests,omitempty"`

	// PackerTests lists the Packer tests to run on that artifact.
	// For example: `["./test/*"]`.
	PackerTests []string `yaml:"packerTests,omitempty"`
}

// GrypeTest is a list of tests to run on images that Skaffold builds.
type GrypeTest struct {
	// Severity is the severity of the vulnerability.
	Severity string `yaml:"severity,omitempty"`
}

// TrivyTest is a list of tests to run on images that Skaffold builds.
...
type DeployType struct {
	...
	// TerraformDeploy deploys a terraform project.
	TerraformDeploy *TerraformDeploy `yaml:"terraform,omitempty"`
}

// TerraformDeploy is a list of terraform deployments to run.
type TerraformDeploy struct {
	// Deployments is a list of terraform deployments to run.
	Deployments []TerrformDeployments `yaml:"deployments,omitempty"`
...
// TerrformDeployments is a list of terraform deployments to run.
type TerrformDeployments struct {
	// Name is The name of the deployment. This is used to create ordering in the terraform deployment.
	Name string `yaml:"name,omitempty"`
	...
	// Region Must be one of the regions listed in https://cloud.google.com/run/docs/locations.
	Region string `yaml:"region,omitempty"`
	// Dir is the directory containing the Terraform configuration files.
	// Defaults to the current working directory.
	Dir string `yaml:"dir,omitempty" skaffold:"filepath"`
	// Vars is a map of variables to be passed to the Terraform command.
	Vars map[string]string `yaml:"vars,omitempty"`
	// VarFiles is a list of variable files to be used by Terraform.
	VarFiles []string `yaml:"varFiles,omitempty" skaffold:"filepath"`
	// BackendConfig is a map of backend configuration to be used by Terraform.
	BackendConfig map[string]string `yaml:"backendConfig,omitempty"`
	// Command is the Terraform command to run. Defaults to "apply".
	Command string `yaml:"command,omitempty"`
	// ExtraArgs are additional arguments to be passed to the Terraform command.
	ExtraArgs []string `yaml:"extraArgs,omitempty"`
	// WorkspaceCommand is the Terraform workspace command to run before the main command.
	// For example: "new" or "select".
	WorkspaceCommand string `yaml:"workspaceCommand,omitempty"`
	// Workspace is the name of the Terraform workspace to use.
	Workspace string `yaml:"workspace,omitempty"`
	// AutoApprove automatically approves Terraform operations without prompting.
	AutoApprove bool `yaml:"autoApprove,omitempty"`
	// LifecycleHooks describes a set of lifecycle host hooks that are executed before and after the Cloud Run deployer.
	LifecycleHooks TerraformDeployHooks `yaml:"hooks,omitempty"`
	// DependsOn is a list of deployments that this deployment depends on.
...
type ArtifactType struct {
	...
	// PackerArtifact builds images using HashiCorp Packer.
	PackerArtifact *PackerArtifact `yaml:"packer,omitempty" yamltags:"oneOf=artifact"`
}

// PackerArtifact builds images using HashiCorp Packer.
type PackerArtifact struct {
	// TemplatePath is the path to the Packer template file.
	TemplatePath string `yaml:"templatePath,omitempty" skaffold:"filepath"`

	// BuildArgs are additional arguments passed to `packer build`.
	// For example: `["-force", "-debug"]`.
	BuildArgs []string `yaml:"buildArgs,omitempty"`

	// PostProcessors specifies which post-processors to use.
	// If empty, all post-processors in the template will be used.
	PostProcessors []string `yaml:"postProcessors,omitempty"`

	// Env are environment variables passed to Packer.
	// For example: `["PKR_VAR_foo=bar", "PKR_VAR_baz=qux"]`.
	Env []string `yaml:"env,omitempty" skaffold:"template"`

	// ContextDir is the directory containing the Packer context.
	// If not specified, it defaults to the directory containing the template file.
	ContextDir string `yaml:"contextDir,omitempty" skaffold:"filepath"`
}
...
// TerraformDeployHooks describes the list of lifecycle hooks to execute in the host before and after the Cloud Run deployer.
